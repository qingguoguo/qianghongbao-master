...
public class WechatAccessbilityJob extends BaseAccessbilityJob {
    ...
    /** 不能再使用文字匹配的最小版本号 */
    private static final int USE_ID_MIN_VERSION = 1221;// 6.3.8 对应code为680,6.3.9对应code为700
    ...
    private void handleLuckyMoneyReceive() {
        ...
        if(nodeInfo == null) {
            Log.w(TAG, "handleLuckyMoneyReceive rootWindow为空");
            return;
        }
    ...
    private void handleChatListHongBao() {
        ...
        AccessibilityNodeInfo nodeInfo = getService().getRootInActiveWindow();
        if(nodeInfo == null) {
            Log.w(TAG, "handleChatListHongBao rootWindow为空");
            return;
        }
